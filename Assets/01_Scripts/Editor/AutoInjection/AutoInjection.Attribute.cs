...
public class ChildField : AutoInjectionField
{
    ...
    public ChildField() { }
    public ChildField(string childName = null) { _childName = childName; }

    Transform GetChildRecursive(Transform target, string name)
    {
        foreach (Transform child in target)
        {
            ...
            var descendant = GetChildRecursive(child, name);
            if (descendant != null)
                return descendant;
        }
        return null;
    ...
    public override bool Inject(MonoBehaviour target, FieldInfo field)
    {
        ...
        var find = GetChildRecursive(target.transform, nameToFind);
        if (find == null)
        {
        ...
        var component = find.GetComponent(field.FieldType);
        if (component == null)
        {
...
public class ChildrenGroupField : AutoInjectionField
{
    ...
    public ChildrenGroupField(string childrenGroupName = null) { _childrenGroupName = childrenGroupName; }
